...
2. **Insertion** — when a window appears, the mouse pointer's position picks the leaf it lands on and the edge it crosses; a new internal node splits that leaf horizontally or vertically, with the existing window on one side and the newcomer on the other. Dropping in the centre stacks the windows.
...
Drag a window to move it. On drop, the edge of the target leaf closest to the cursor determines the new split direction; dropping in the centre adds the window to the target's stack. Drags work across screens and Spaces — the target Space's BSP is updated transparently.
